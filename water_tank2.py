...
def computer_play(computer_tank, computer_cards, water_cards_pile, power_cards_pile, human_tank):
    def computer_play(computer_tank, computer_cards, water_cards_pile, power_cards_pile, opponent_tank):
        ...
        use_or_discard = random.randint(0, 3)  # 75% chance card is used.

        if use_or_discard in (0, 1, 2):
            if computer_tank >= 74:
                card_to_use = min(water_cards_pile)
            elif 45 < opponent_tank < 75:
                if "DOT" in computer_cards:
                    card_to_use = "DOT"
                elif "SOH" in computer_cards:
                    card_to_use = "SOH"
                else:
                    card_to_use = random.choice(computer_cards)
            elif computer_tank == 0:
                card_to_use = max(water_cards_pile)
            elif 40 > computer_tank > 20:
                if "SOH" in computer_cards:
                    card_to_use = "SOH"
                else:
                    card_to_use = random.choice(computer_cards)
            elif 25 < opponent_tank > computer_tank:
                if "DOT" in computer_cards:
                    card_to_use = "DOT"
                else:
                    card_to_use = random.choice(computer_cards)
            elif computer_tank == 40:
        ...
        else:  # 25% card is discarded
            if 1 in computer_cards:
                discard = 1
                discard_card(discard, computer_cards, water_cards_pile, power_cards_pile)
                print("Computer is now discarding card: {}".format(discard))
                get_card_from_pile("water", 0)
            elif "DOT" not in computer_cards:  # Always want to attempt to have DOT in hand for strategy
                discard = random.choice(computer_cards)
                discard_card(discard, computer_cards, water_cards_pile, power_cards_pile)
                print("Computer is now discarding card: {}".format(discard))
                get_card_from_pile("power", 0)

        if use_or_discard in (0, 1, 2):
            draw_card = water_cards_pile.pop(0)
        else:
            draw_card = power_cards_pile.pop(0)
    ...
    should
    work
    with the variables and game structure you provided earlier.It employs a defensive strategy based on various conditions, such as the computer's tank level, opponent's tank level, and card availability.It also ensures that the computer's hand is properly managed and updated.
